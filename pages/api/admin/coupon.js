    ...
    await authMiddleware(req, res, async () => {
      ...
      if (method === "POST") {
        ...
        if (checkCoupon) {
          return res
            .status(400)
            .json({ error: "Coupon already exists, try a different name.", ok: false });
        }
        ...
        if (coupon.length < 4 || coupon.length > 10) {
          return res
            .status(400)
            .json({ error: "Coupon length should be between 4 to 10 characters.", ok: false });
        }
        ...
        if (convertedStartDate < Date.now()) {
          return res
            .status(400)
            .json({ error: "Start date cannot be less than today's date.", ok: false });
        }
        ...
        if (startDate > endDate) {
          return res
            .status(400)
            .json({ error: "Start date cannot be greater than end date.", ok: false });
        }
      ...
      if (method === "PATCH") {
        ...
        return res
          .status(200)
          .json({ message: "Coupon updated successfully.", coupons: coupons, ok: true });
      }
      ...
      if (method === "PUT") {
        ...
        // send response
        return res.status(200).json({ message: "Coupon deleted successfully.", coupons, ok: true });
      }
    });
  } catch (error) {
